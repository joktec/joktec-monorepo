...
export const BaseController = <T extends Entity, ID>(props: IControllerProps<T>): Type<IBaseController<T, ID>> => {
  ...
  class Controller implements IBaseController<T, ID>, OnModuleInit {
    ...
    @ApiExcludeEndpoint(props.paginate?.hidden)
    ...
    @ApiOkResponse({ type: PaginationDto })
    @ApiExcludeEndpoint(props.paginate?.hidden && !props.paginate?.usePost)
    ...
    @ApiExcludeEndpoint(props.detail?.hidden)
    ...
    @ApiExcludeEndpoint(props.create?.hidden)
    ...
    @ApiExcludeEndpoint(props.update?.hidden)
    ...
    @ApiOkResponse({ type: props.dto })
    @ApiParam({ name: 'id' })
    @ApiExcludeEndpoint(props.delete?.hidden)
